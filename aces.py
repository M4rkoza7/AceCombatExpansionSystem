...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.delete_btn.clicked.connect(self.confirm_delete_plane)
        select_row.addWidget(self.delete_btn)
        ...
        self.setWindowTitle("ACES - Ace Combat Expansion System")
        try:
    ...
    def toggle_mode(self, checked):
        ...
        self.delete_btn.setVisible(checked)

    def confirm_delete_plane(self):
        plane_name = self.existing_combo.currentText()
        if not plane_name:
            QtWidgets.QMessageBox.warning(self, "Delete Plane", "No plane selected.")
            return

        total_planes = self.existing_combo.count()
        if total_planes <= 1:
            QtWidgets.QMessageBox.information(
                self,
                "Delete Plane",
                "Cannot delete the last remaining plane entry."
            )
            return
        ...
        reply = QtWidgets.QMessageBox.question(
            self,
            "Confirm Delete",
            f"Are you sure you want to delete '{plane_name}' from the datatables? "
        ...
        if reply == QtWidgets.QMessageBox.StandardButton.Yes:
            self.delete_selected_plane(plane_name)

    def delete_selected_plane(self, plane_name):
        self.setEnabled(False)
        try:
            total_planes = self.existing_combo.count()
            current_index = self.existing_combo.currentIndex()

            # Determine which plane to select after deletion
            next_index = current_index - 1 if current_index > 0 else (
                current_index + 1 if total_planes > 1 else -1
            )
            ...
            if next_index == -1:
                QtWidgets.QMessageBox.information(
                    self,
                    "Delete Plane",
                    "Cannot delete the only remaining plane entry."
                )
                return
            ...
            # --- 1️⃣ Delete the plane from all three datatables ---
            for table_name in ["PlayerPlaneDataTable", "SkinDataTable", "AircraftViewerDataTable"]:
                path = resolve_input_file(table_name, data_dir)
                json_path = normalize_to_json(path)
                ...
                data_array = data["Exports"][0]["Table"]["Data"]
                new_data = [
                    entry for entry in data_array
                    if not any(
                        p.get("Name") == "PlaneStringID" and p.get("Value") == plane_name
                        for p in entry.get("Value", [])
                    )
                ]
                removed_count = len(data_array) - len(new_data)

                if removed_count > 0:
                    self.log.append(f"Removed {removed_count} entry(ies) from {table_name} for plane '{plane_name}'.")

                data["Exports"][0]["Table"]["Data"] = new_data
                save_json(data, os.path.join(output_dir, f"{table_name}.json"))

            # --- 2️⃣ Log completion ---
            self.log.append(f"Plane '{plane_name}' successfully deleted from all datatables.")

            # --- 3️⃣ Update dropdown list ---
            self.existing_combo.removeItem(current_index)
            if self.existing_combo.count() > 0:
                new_index = max(0, next_index if next_index < self.existing_combo.count() else self.existing_combo.count() - 1)
                self.existing_combo.setCurrentIndex(new_index)
            else:
                QtWidgets.QMessageBox.information(
                    self,
                    "Delete Plane",
                    "All planes have been removed. Please re-import or add new entries."
                )
                return

            # --- 4.5 Update UI input paths to point to the newly-saved files in Output/ ---
            pp_out = os.path.join(output_dir, "PlayerPlaneDataTable.json")
            sk_out = os.path.join(output_dir, "SkinDataTable.json")
            av_out = os.path.join(output_dir, "AircraftViewerDataTable.json")
            
            # Point the UI at the Output files so start_pipeline() will use them
            self.pp_input_edit.setText(pp_out)
            self.skin_input_edit.setText(sk_out)
            self.av_input_edit.setText(av_out)
            self.data_dir_edit.setText(output_dir)
            
            # Ensure Qt processes the UI updates immediately (important before starting the worker)
            QtWidgets.QApplication.processEvents()
            
            # Now run the pipeline — start_pipeline will read the updated UI paths and use Output/.
            self.log.append("Running export pipeline after deletion...")
            self.start_pipeline()
            ...
            QtWidgets.QMessageBox.critical(self, "Delete Plane", f"Failed to delete plane: {e}")
            self.log.append(f"Error deleting plane '{plane_name}': {e}")
